...
export class ReviewController {
  ...
  async listMyReviews(req: Request, res: Response, next: NextFunction): Promise<void> {
    try {
      const user = (req as AuthenticatedRequest).user;
      if (!user) {
        next(new ApiError('Authentication required', 401, 'AUTH_REQUIRED'));
      ...
      const sort = typeof req.query.sort === 'string' ? req.query.sort : 'createdAt';
      const order = typeof req.query.order === 'string' ? req.query.order : 'desc';
      const result = await reviewService.listMyReviews(user.sub, page, limit, sort, order as 'asc' | 'desc');
      sendSuccess(res, result, 'My reviews fetched');
    } catch (error) {
      next(error);
    }
  }
