...
const navItems = [
  ...
  { href: '/patient/allergies', label: 'Allergies' },
...
export function PatientShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_top_left,_#effaf4,_#f8fafc_45%,_#eef2ff)] text-slate-900">
      <div className="mx-auto grid min-h-screen max-w-7xl gap-6 px-4 py-4 sm:px-6 lg:grid-cols-[280px_minmax(0,1fr)] lg:px-8">
        <aside className="flex flex-col rounded-[28px] border border-slate-200/80 bg-white/90 p-5 shadow-[0_20px_60px_-30px_rgba(15,23,42,0.35)] backdrop-blur">
          <div className="rounded-[22px] bg-slate-950 p-5 text-white">
            <p className="text-xs font-semibold uppercase tracking-[0.35em] text-emerald-300">Patient portal</p>
            <h1 className="mt-3 text-2xl font-semibold">DocDock</h1>
            <p className="mt-2 text-sm text-slate-300">Personal health hub for your profile, records, and preferences.</p>
          </div>
          ...
          <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
            ...
            <p className="mt-1 break-words">{user?.fullName || 'Patient'}</p>
          ...
          <header className="relative z-30 rounded-[28px] border border-slate-200/80 bg-white/90 px-5 py-4 shadow-[0_20px_60px_-30px_rgba(15,23,42,0.28)] backdrop-blur sm:px-6">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">Manage your care profile from one place.</p>
              </div>
              <div className="flex flex-wrap items-center gap-3 text-sm">
                <NotificationBell />
                <Link href="/find-doctors" className="rounded-full border border-slate-300 px-4 py-2 font-medium text-slate-700 transition hover:bg-slate-100">
                  Find doctors
                </Link>
                <Link href="/" className="rounded-full bg-slate-950 px-4 py-2 font-medium text-white transition hover:bg-slate-800">
                  Home
                </Link>
              </div>
            </div>
